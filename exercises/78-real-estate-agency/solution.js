...
class Property {
  constructor(id, type, area, price, bedrooms, location, listedDay, agentId) {
    this.id = id;
    this.type = type; // 'apartment', 'house', 'commercial'
    this.area = area; // m²
    this.price = price;
  ...
  getPricePerSqMeter() {
    // CORREGIDO: dividir precio entre área, no área entre precio
...
class Transaction {
  ...
  getDiscountPercentage() {
    // CORREGIDO: el denominador debe ser listedPrice (precio de referencia), no salePrice
    return ((this.listedPrice - this.salePrice) / this.listedPrice) * 100;
  }
...
class RealEstateAgency {
  ...
  getAverageSalePrice() {
    ...
    // CORREGIDO: usar salePrice (precio real de venta), no listedPrice
    return this.transactions.reduce((sum, t) => sum + t.salePrice, 0) / this.transactions.length;
  }
  ...
  getTopAgents(n) {
    ...
    // CORREGIDO: ordenar descendente (b.totalSales - a.totalSales)
    return agentStats.sort((a, b) => b.totalSales - a.totalSales).slice(0, n);
  }
