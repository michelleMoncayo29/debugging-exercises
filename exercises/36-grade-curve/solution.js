...
function applyCurve(scores) {
  // CORREGIDO: la curva debe escalar dividiendo entre el MÁXIMO, no entre el
  // promedio. Dividir entre el promedio distorsiona las proporciones y puede
  // subir la nota máxima por encima de 100.
  const max = Math.max(...scores);

  if (max === 0) return scores.map(() => 0);

  return scores.map(score => {
    const curved = (score / max) * 100;
