...
function checkoutBook(catalog, bookId, userId) {
  // CORREGIDO: Primero se verifica si el libro existe antes de acceder a sus propiedades
...
function returnBook(catalog, bookId) {
  ...
  // CORREGIDO: Usa book.available para verificar si está disponible,
  // sin intentar acceder a book.checkedOutBy (que puede ser null) como string
  if (book.available) {
...
function getStats(catalog) {
  ...
  // CORREGIDO: Filtra usando b.available (campo correcto) en lugar de !b.checkedOut (campo inexistente)
